...
const HEADERS: Record<string, string> = {
  "User-Agent": "Mozilla/5.0 (compatible; WESO-DexTools-Adapter/1.0)",
...
export async function getBlockByNumber(height: number): Promise<LatestBlockInfo> {
  const res = await fetchJson<{
    block: { header: { height: string; time: string } };
  }>(`${LCD_URL}/cosmos/base/tendermint/v1beta1/blocks/${height}`);
  const blockNumber = Number(res.block.header.height);
  const blockTimestamp = Math.floor(Date.parse(res.block.header.time) / 1000);
  if (!Number.isFinite(blockNumber) || !Number.isFinite(blockTimestamp)) {
    throw new LcdError(`Invalid block payload for height ${height}`);
  }
  return { blockNumber, blockTimestamp };
}

/**
 * Youngest block with timestamp <= requested (seconds). Binary search on height.
 */
export async function getBlockByTimestamp(timestamp: number): Promise<LatestBlockInfo> {
  const latest = await getLatestBlock();
  if (timestamp >= latest.blockTimestamp) return latest;
  if (timestamp <= 0) {
    throw Object.assign(new Error("timestamp must be positive"), { status: 400 });
  }
  let lo = 1;
  let hi = latest.blockNumber;
  let best: LatestBlockInfo | null = null;
  while (lo <= hi) {
    const mid = Math.floor((lo + hi) / 2);
    try {
      const b = await getBlockByNumber(mid);
      if (b.blockTimestamp <= timestamp) {
        best = b;
        lo = mid + 1;
      } else {
        hi = mid - 1;
  ...
  if (!best) {
    throw Object.assign(new Error(`No block found at or before timestamp ${timestamp}`), {
      status: 404,
    });
  }
  return best;
